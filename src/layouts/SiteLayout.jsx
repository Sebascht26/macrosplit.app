...
import ExploreTools from "../components/ExploreTools.jsx";
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="mx-auto max-w-6xl px-4 h-14 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-2 text-sm">
            <Nav to="/" end>Home</Nav>
            <Nav to="/calories">Calories</Nav> {/* ← new */}
...
function Nav({ to, end = false, children }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `px-2 py-1 rounded-md ${isActive ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-100"}`
      }
    >
      {children}
    </NavLink>
